...
export const SchematicViewer = ({ fileUrl }) => {
    ...
    if (!fileUrl) {
        return (
            <div className="h-[65%] flex flex-col items-center justify-center bg-gray-50 text-gray-400">
                <Loader className="animate-spin mb-2" size={32} />
                <p className="text-sm">No schematic file found</p>
            </div>
        );
    }

    return (
        <div ref={containerRef} className="h-[65%] w-full">
            <kicanvas-embed
                src={fileUrl}
                controls="full"
